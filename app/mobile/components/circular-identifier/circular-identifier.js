CircularIdentifier = Component.create({
  ...
  containerTracker: function(){
    ...
    var containerWidth = parseInt(this.subElm('.arc-container').css('width'));
    var containerHeight = parseInt(this.subElm('.arc-container').css('height'));
    var left = this.source.x - arcWidth/2;
    var top = this.source.y - containerHeight + arcWidth/2;

    this.subElm('.arc-container').css({left:left, top:top});
      ...
      self.lineHand.animate({path:stage2}, 200, function(){
        self.lineHand.animate({path:stage3}, 200, function(){
          self.lineHand.animate({path:stage4}, 200, function(){
            self.subElm('.text').transition({width:425},350);
            self.infoRect.animate({width:500}, 300, function(){
  ...
  {
    var self = this;
    var raphaelSVG = this.subElm('.arc-container .hand-arc');
